...
import { fetchAgentDetailsWithUniqueId } from '@/api/auth'
...
function AgentVerification({
    ...
    if(!id){
        navigator.push('/agent-verification')
        return
    }
    ...
    return (
        <main>
            ...
            {data && data?.workingStatus === "working" && <AgentAvailabilityCard
                success
            ...
            {data && data?.workingStatus === "not-working" && <AgentAvailabilityCard
                error
            ...
            {!data && <AgentAvailabilityCard
                error
                title='This agent is not in our records.'
                subtitle={
                    <p className='max-w-[480px] w-full m-auto  text-center'>We could not verify this Agent ID. This agent is not affiliated with PropertySeller.
...
function Button() {
    return (
        ...
        className='border-[#DEDEDE] cursor-pointer flex px-4 py-2 gap-3 border rounded-[5px]'
        >
            <LuMoveLeft />
