...
import MobileEditProfileHeader from "./mobile/MobileEditProfileHeader";
...
function RegistrationComponent() {
  ...
  const {
    data: fectchedUserProfileDetails,
    isLoading: isFetchingUserProfileDetails,
  } = useFetchUserProfileDetailsQuery();
...
<Container>
        <section className="flex flex-col min-h-[90vh] pb-20 items-start justify-center bg-white">
          <p className="font-poppins max-w-[400px] mt-6 font-medium text-[19.5px]  sm:text-[24px]">
            Edit Profiler
          </p>
          <Formik
            enableReinitialize
            ...
            {({}) => (
              <Form className="w-full max-w-[400px] mt-4 smmt-6 bg-white">
                ...
                <button
                  ...
                  className="w-full mt-3 text-[14px] cursor-pointer font-medium bg-[#FF1645] text-white  h-[35px] rounded-[3px] hover:bg-[#D8133A] transition flex items-center justify-center"
                >
                  {isSubmitting ? (
                    <svg
